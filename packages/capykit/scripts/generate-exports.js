...
function findComponentDirs() {
  ...
  for (const item of items) {
    ...
    const indexDtsExists = fs.existsSync(path.join(componentPath, 'index.d.ts'));
    const vueFileExists = fs.readdirSync(componentPath).some(file => file.endsWith('.vue'));
    ...
    if (indexJsExists && indexDtsExists && vueFileExists) {
      ...
      const vueFiles = fs.readdirSync(componentPath).filter(file => file.endsWith('.vue'));
...
function updatePackageJsonExports(components) {
  ...
  try {
    ...
    console.log(`✅ Updated package.json exports for ${components.length} components`);
  } catch (error) {
...
function updateIndexJs(components) {
  ...
  try {
    ...
    console.log(`✅ Updated index.js with ${components.length} component exports`);
  } catch (error) {
...
function updateIndexDts(components) {
  ...
  try {
    ...
    console.log(`✅ Updated index.d.ts with ${components.length} component exports`);
  } catch (error) {
...
try {
  ...
  if (components.length === 0) {
    ...
    console.log('Components must have: index.js, index.d.ts, and at least one .vue file');
    process.exit(0);
  }
  ...
  console.log('\n📝 Summary:');
  console.log(`   - Updated package.json with ${components.length} export paths`);
  console.log(`   - Updated index.js with re-exports`);
  console.log(`   - Updated index.d.ts with type exports`);
  console.log('\n💡 Available imports:');
  console.log(`   import { ${components.map(c => c.componentName).join(', ')} } from "capykit"`);
  components.forEach(comp => {
    console.log(`   import { ${comp.componentName} } from "capykit/${comp.dirName}"`);
  });
