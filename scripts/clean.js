...
const cleanupTasks = [
  {
    ...
    description: 'Removes the /packs directory and all its contents',
    path: 'packs',
...
function runCleanup() {
  ...
  if (enabledTasks.length === 0) {
    ...
    console.log('Enable tasks in scripts/clean.js to perform cleanup operations');
    return;
  }
  ...
  if (disabledTasks.length > 0) {
    console.log(`${disabledTasks.length} additional cleanup tasks available (disabled):`);
  ...
  console.log(`Summary: ${completedTasks}/${totalTasks} tasks completed, ${skippedTasks} skipped`);
  
  if (completedTasks < totalTasks) {
    console.log('Some cleanup tasks failed. Check the output above for details.');
    process.exit(1);
  }
}
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1].endsWith('clean.js')) {
  runCleanup();
}